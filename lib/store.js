...
const extendSewaGroup = (gid, additionalTime, _dir) => {
    ...
    if (position === null) return false; // Jika grup tidak ditemukan, return false

    // Perbarui waktu kedaluwarsa dengan menambahkan waktu tambahan
    _dir[position].expired += toMs(additionalTime);
    
    // Tulis kembali ke sewa.json untuk menyimpan perubahan
    fs.writeFileSync('./database/sewa.json', JSON.stringify(_dir, null, 2));
    return true;
};
...
const expiredCheck = (alpha, _dir) => {
    setInterval(() => {
        let expiredGroups = []; // Track all expired group positions

        // Find all expired groups
        _dir.forEach((entry, index) => {
        ...
        expiredGroups.forEach(async (index) => {
            const groupId = _dir[index].id;
            console.log(`Sewa expired: ${groupId}`);

            try {
                await alpha.sendMessage(groupId, { text: `Masa sewa di grup ini telah habis, bot otomatis keluar!` });
                await alpha.groupLeave(groupId);

                // Remove expired group from the array and update the database
                _dir.splice(index, 1);
                fs.writeFileSync('./database/sewa.json', JSON.stringify(_dir, null, 2));
            } catch (error) {
    ...
    }, 1000 * 60); // Check every minute to reduce frequent file writes
};
